...
import br.com.senior.mydomain.myservice.FindProductStockGreaterInput;
import br.com.senior.mydomain.myservice.FindProductStockGreaterOutput;
 ...
 * 
 */
public class FindProductStockGreaterImpl {
	...
	public FindProductStockGreaterImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindProductStockGreaterInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_PRODUCT_STOCK_GREATER, //
	...
	*/
	public FindProductStockGreaterOutput findProductStockGreater(FindProductStockGreaterInput input, long timeout) {
		...
		FindProductStockGreaterOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindProductStockGreaterOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindProductStockGreaterOutput> findProductStockGreaterRequest(FindProductStockGreaterInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductStockGreaterOutput.class);
	}
	...
	* Query destinada para retornar Lista de Produtos com estoque maior que o valor passado no parametro stock
	*/
	public CompletableFuture<FindProductStockGreaterOutput> findProductStockGreaterRequest(FindProductStockGreaterInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductStockGreaterOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
