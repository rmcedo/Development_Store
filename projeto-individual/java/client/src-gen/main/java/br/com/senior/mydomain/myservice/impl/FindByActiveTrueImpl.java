...
import br.com.senior.mydomain.myservice.FindByActiveTrueInput;
import br.com.senior.mydomain.myservice.FindByActiveTrueOutput;
 ...
 * 
 */
public class FindByActiveTrueImpl {
	...
	public FindByActiveTrueImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindByActiveTrueInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_BY_ACTIVE_TRUE, //
	...
	*/
	public FindByActiveTrueOutput findByActiveTrue(FindByActiveTrueInput input, long timeout) {
		...
		FindByActiveTrueOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindByActiveTrueOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindByActiveTrueOutput> findByActiveTrueRequest(FindByActiveTrueInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByActiveTrueOutput.class);
	}
	...
	* Query destinada para encontrar Clientes que estão ativos ou não dentro do sistema
	*/
	public CompletableFuture<FindByActiveTrueOutput> findByActiveTrueRequest(FindByActiveTrueInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByActiveTrueOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
