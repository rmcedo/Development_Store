 ...
 * 
 */
public class RetrieveClientImpl {
	...
	public RetrieveClientImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Client.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_CLIENT, //
	...
	*/
	public Client retrieveClient(Client.GetRequest input, long timeout) {
		...
		Client output = DtoJsonConverter.toDTO(resultMessage.getContent(), Client.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Client> retrieveClientRequest(Client.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Client.class);
	}
	...
	* The 'retrieve' request primitive for the Client entity.
	*/
	public CompletableFuture<Client> retrieveClientRequest(Client.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Client.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
