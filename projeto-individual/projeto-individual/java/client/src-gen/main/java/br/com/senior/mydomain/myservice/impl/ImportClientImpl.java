...
import br.com.senior.mydomain.myservice.ImportClientInput;
import br.com.senior.mydomain.myservice.ImportClientOutput;
...
public class ImportClientImpl {
	...
	public ImportClientImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportClientInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_CLIENT, //
	...
	*/
	public ImportClientOutput importClient(ImportClientInput input, long timeout) {
		...
		ImportClientOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportClientOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportClientOutput> importClientRequest(ImportClientInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportClientOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportClientOutput> importClientRequest(ImportClientInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportClientOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
