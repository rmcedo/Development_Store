...
import br.com.senior.mydomain.myservice.FindByCpfInput;
import br.com.senior.mydomain.myservice.FindByCpfOutput;
...
public class FindByCpfImpl {
	...
	public FindByCpfImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindByCpfInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_BY_CPF, //
	...
	*/
	public FindByCpfOutput findByCpf(FindByCpfInput input, long timeout) {
		...
		FindByCpfOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindByCpfOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindByCpfOutput> findByCpfRequest(FindByCpfInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByCpfOutput.class);
	}
	...
	* Query destinada para retornar Cliente com CPF igual ao passado como parametro
	*/
	public CompletableFuture<FindByCpfOutput> findByCpfRequest(FindByCpfInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByCpfOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
