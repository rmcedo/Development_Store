...
import br.com.senior.mydomain.myservice.FindProductPriceLessInput;
import br.com.senior.mydomain.myservice.FindProductPriceLessOutput;
 ...
 * 
 */
public class FindProductPriceLessImpl {
	...
	public FindProductPriceLessImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindProductPriceLessInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_PRODUCT_PRICE_LESS, //
	...
	*/
	public FindProductPriceLessOutput findProductPriceLess(FindProductPriceLessInput input, long timeout) {
		...
		FindProductPriceLessOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindProductPriceLessOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindProductPriceLessOutput> findProductPriceLessRequest(FindProductPriceLessInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductPriceLessOutput.class);
	}
	...
	* Query destinada para retornar Lista de Produtos com preço menor que o valor passado no parametro price
	*/
	public CompletableFuture<FindProductPriceLessOutput> findProductPriceLessRequest(FindProductPriceLessInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductPriceLessOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
