...
import br.com.senior.mydomain.myservice.FindAllRecordOutput;
 ...
 * 
 */
public class FindAllRecordImpl {
	...
	public FindAllRecordImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
	...
	private Message createMessage() {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				...
				MyServiceConstants.Commands.FIND_ALL_RECORD, //
				new byte[0]);
		}
	...
	*/
	public FindAllRecordOutput findAllRecord(long timeout) {
		...
		FindAllRecordOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindAllRecordOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindAllRecordOutput> findAllRecordRequest() {
		...
		Message message = createMessage();
		addMessageHeaders(message);
		return serviceRunner.request(message, FindAllRecordOutput.class);
	}
	...
	* Query destinada para retornar lista de produtos contendo preços, VIA RECORD
	*/
	public CompletableFuture<FindAllRecordOutput> findAllRecordRequest(long timeout, TimeUnit unit) {
		...
		Message message = createMessage();
		addMessageHeaders(message);
		return serviceRunner.request(message, FindAllRecordOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
