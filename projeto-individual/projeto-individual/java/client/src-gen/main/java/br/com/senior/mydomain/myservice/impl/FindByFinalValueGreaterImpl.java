...
import br.com.senior.mydomain.myservice.FindByFinalValueGreaterInput;
import br.com.senior.mydomain.myservice.FindByFinalValueGreaterOutput;
 ...
 * 
 */
public class FindByFinalValueGreaterImpl {
	...
	public FindByFinalValueGreaterImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindByFinalValueGreaterInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_BY_FINAL_VALUE_GREATER, //
	...
	*/
	public FindByFinalValueGreaterOutput findByFinalValueGreater(FindByFinalValueGreaterInput input, long timeout) {
		...
		FindByFinalValueGreaterOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindByFinalValueGreaterOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindByFinalValueGreaterOutput> findByFinalValueGreaterRequest(FindByFinalValueGreaterInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByFinalValueGreaterOutput.class);
	}
	...
	* Query destinada para retornar Lista de Pedidos com valores finais com maior valor do que o parametro passado
	*/
	public CompletableFuture<FindByFinalValueGreaterOutput> findByFinalValueGreaterRequest(FindByFinalValueGreaterInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindByFinalValueGreaterOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
