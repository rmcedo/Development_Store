...
import br.com.senior.mydomain.myservice.FindProductStockLessInput;
import br.com.senior.mydomain.myservice.FindProductStockLessOutput;
 ...
 * 
 */
public class FindProductStockLessImpl {
	...
	public FindProductStockLessImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindProductStockLessInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_PRODUCT_STOCK_LESS, //
	...
	*/
	public FindProductStockLessOutput findProductStockLess(FindProductStockLessInput input, long timeout) {
		...
		FindProductStockLessOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindProductStockLessOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindProductStockLessOutput> findProductStockLessRequest(FindProductStockLessInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductStockLessOutput.class);
	}
	...
	* Query destinada para retornar Lista de Produtos com estoque menor que o valor passado no parametro stock
	*/
	public CompletableFuture<FindProductStockLessOutput> findProductStockLessRequest(FindProductStockLessInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindProductStockLessOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
