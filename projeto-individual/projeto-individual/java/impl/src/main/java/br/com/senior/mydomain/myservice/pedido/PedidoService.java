...
//    }
    ...
    public Product convertProduct(ProductEntity p){
        return new Product(p.getId().toString(),p.getName(),p.getDescription(),p.getPrice(),p.getStock(),p.getActive(),p.getCategoria(),null);
    }
    ...
    public List<PedidoEntity> findByFinalValueGreater(BigDecimal value){

        return repository.findByFinalValueGreaterThan(value);
